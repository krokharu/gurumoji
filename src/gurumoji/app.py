...
def group_analysis_for_row(
    ...
    *,
    ...
    transformer_saved = json_load(row["transformer_analysis_json"], {}) \
    ...
    analysis["transformer"] = {
        ...
        "stale": bool(transformer_result) and transformer_result.get("fingerprint") != transformer_input_fingerprint(
...
def lmstudio_base_url(value: str) -> str:
    """Return the safe, canonical OpenAI-compatible LM Studio base URL.

    The local provider intentionally never accepts a LAN or Internet address:
    transcription text must remain on the same computer unless a cloud provider
    is selected explicitly.  IP literals also avoid DNS rebinding concerns.
    """
    raw = str(value or "").strip() or LMSTUDIO_DEFAULT_BASE_URL
    try:
    ...
    if (
        ...
        or host not in LMSTUDIO_LOOPBACK_HOSTS
...
def audio_preprocess_filters(preset: str) -> list[str]:
    if preset not in AUDIO_PREPROCESS_PRESETS:
        raise RuntimeError(f"未対応の音声前処理です: {preset}")
    preset_filters = AUDIO_PREPROCESS_PRESETS[preset]["filters"]
    if not preset_filters:
        return []
    # Normalize the final channel mix, and keep the FFT hop size independent
    # of the source sample rate. Floating point preserves quiet input samples.
    filters = [
        f"aresample={WHISPER_SAMPLE_RATE}",
        "aformat=sample_fmts=fltp:channel_layouts=mono",
    ]
    for audio_filter in preset_filters:
        if audio_filter.startswith("afftdn="):
            # afftdn delays samples by two 12.5 ms hops without extending EOF.
            # Flush its tail, remove the delayed prefix, then reset timestamps
            # before subsequent filters so ASR stays aligned with the media.
            delay_samples = 2 * (WHISPER_SAMPLE_RATE // 80)
            filters.extend([
                f"apad=pad_len={delay_samples}",
                audio_filter,
                f"atrim=start_sample={delay_samples}",
...
def run_audio_preprocess(
    ...
    filters = audio_preprocess_filters(preset)
    if not filters:
        return input_path
...
def run_audio_interval_preprocess(
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    # Limit the decoded input before loudnorm buffers it. An output-side -t
    # can truncate its final buffered frame and lose part of the interval.
    command = [
        "ffmpeg",
        "-hide_banner",
        "-nostdin",
...
def call_ai_json(
    ...
    if provider == "openai":
        ...
        endpoint = lmstudio_base_url(base_url)
        level = normalize_efforts(ai_efforts).get(SCHEMA_STAGES.get(schema_name), "auto")
        if level != "auto":
            reasoning = local_effort_payload(level, lmstudio_reasoning_settings(base_url, api_key, model))
        response = post_json(
            f"{endpoint}/chat/completions",
            lmstudio_headers(api_key),
            {
                "model": lmstudio_model_id(model),
                "messages": [
                    {"role": "system", "content": system_prompt},
    ...
    if provider == "openai":
        ...
        text = extract_lmstudio_text(response)
    try:
...
def clean_segments_with_ai(
    ...
    def call(system, prompt, name, schema):
        return call_ai_json(provider, api_key, model, system, prompt, name, schema,
                            check_cancelled, usage_callback, base_url, **({"ai_efforts": ai_efforts} if ai_efforts else {}))
...
def create_outline_with_ai(
    ...
    def call(system, prompt, name, schema):
        return call_ai_json(provider, api_key, model, system, prompt, name, schema,
                            check_cancelled, usage_callback, base_url, **({"ai_efforts": ai_efforts} if ai_efforts else {}))
    result = finish_create_outline(segments, speaker_names, call, status, check_cancelled)
    result["provenance"].update({"provider": provider, "model": model, "generated_at": utc_now_iso()})
    return result
