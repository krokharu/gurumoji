...
const effortStages = [
  ...
  ['cleanup', '再校正・ノイズ判定', '全文と前後の文脈を参照して確認', 'clean-transcript'],
...
if (effortRoot) {
  effortStages.forEach(([key, title, detail]) => {
    ...
    card.innerHTML = `<legend>${title}</legend><p>${detail}</p><input type="hidden" name="ai_effort_${key}" value="medium"><span class="effort-group-title">思考モード</span><div class="thinking-mode effort-options"><label><input type="radio" name="ai_thinking_mode_${key}" value="on" checked><span>ON</span></label><label><input type="radio" name="ai_thinking_mode_${key}" value="off"><span>OFF</span></label></div><span class="effort-group-title">エフォート</span><div class="effort-options">${effortLevels.map(([value, label]) =>
  ...
  try {
    ...
    effortStages.forEach(([key]) => {
      ...
      if (value === 'off') card.querySelector('input[name="ai_thinking_mode_' + key + '"][value="off"]').checked = true;
...
function syncAiEffortSettings() {
  ...
  effortStages.forEach(([key, , , toggle]) => {
    const card = effortRoot?.querySelector(`[data-stage="${key}"]`);
    if (!card) return;
    const enabled = key === 'cleanup' ? obsidian || document.getElementById(toggle)?.checked
