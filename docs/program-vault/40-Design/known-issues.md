...
| OBS-11 | 低中 | 会話を削除した後も、ResearchVaultの概要・一覧が「保存済み」のまま | `_delete_library_item_locked` は `retire_input_vault` だけを呼び、`interviews.json` と概要ノートは更新しない | 削除済みの会話を判別できない | 状態を「アプリから削除済み」に更新する（ノートは来歴として残す） | 対応済み（2026-09-25。`ObsidianLayout.mark_deleted` が `interviews.json` の状態と `deleted_at`、概要ノート、インタビュー一覧を更新する。概要が未作成の会話では何も作らない。その後の仕上げ状態で上書きしない） | P1：優先 |
| OBS-12 | 低中 | 分析ノートに `file:///` の絶対パスリンクを書く | `AnalysisStore.publish` の `local_links` | 端末に依存する。Vaultを共有するとPCのユーザー名などが残る。移動するとリンクが切れる | アプリ経由のURLだけにする。既存のノートは再生成で置き換える（所有hashが一致するものだけ） | 対応済み（2026-09-25。新しく書くノートは「保存先 `analysis_store/…`」の相対表記にした。既存ノートは「Vaultに再保存」で置き換わる。一括置換は未実施） | P1：優先 |
