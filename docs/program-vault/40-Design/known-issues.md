...
**現在の状態（2026-09-25）：** 登録50件（BUG-03を追加）のうち、対応済み23件、一部対応1件（OBS-15）、未対応22件、状態欄が空のもの4件（DEV-01、UI-01〜03）。今回は、P1の未対応だったOBS-03／09／11／12／18とDATA-02／03、および新規のBUG-03に対応した。DATA-01（元メディアのゴミ箱）は、削除してもディスク容量が空かなくなる挙動の変更を伴うため、利用者の判断待ち。残りの未対応は、収束計画で「保留」とした設計上の項目。
