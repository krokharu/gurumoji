...
順序2はAI見解から着手した。保存済み見解と最新requestを1つのSQLite snapshotで読む契約を`AnalysisQueries.insights`へ移し、取得操作でAIを再実行しない境界を維持した。開始・取消も`AnalysisCommands`へ移し、request ID再送、同時実行拒否、revision照合、専門家ゲート、thread開始失敗時の後始末、取消eventを既存の共有lockとDB状態のまま保持した。worker確定、Transformer、分類は未移行。
