...
- 起動時の `recover`・`migrate` が失敗すると、監視スレッドは停止する。状態（`not_started`／`starting`／`running`／`stopped`）、停止理由、ポーリングの直近のエラーを `WatcherStatus` が保持し、`GET /api/config` の `runtime.obsidian_watcher` と「接続と処理装置」画面の「Obsidian監視」に表示する。理由はエラーの種類と要約だけで、ファイルパスやノート本文は含めない（OBS-09）。
- 人が編集した生成ノートや未管理の既存ノートの更新を見送ったときは、`managed_note` がパスと理由をプロセスごとに1回だけログに出す（OBS-15。JSONL監査ログは作らない）。
...
| OBS-03 | 中 | `.obsidian` の設定をアプリが変更する（対応済み 2026-09-25：新規Vaultの初回だけ） |
| OBS-10 | 中 | Vaultの入れ子の痕跡があるのに、検出していない |
| OBS-06／07 | 中 | frontmatterの実装が複数あり、全体を再シリアライズする |
| OBS-09／15 | 中 | 監視の停止とスキップが見えない（OBS-09対応済み、OBS-15はスキップのログのみ。2026-09-25） |
| OBS-11／12／13／14 | 低〜中 | 削除後の表示、`file:///` 絶対パス、移行時の空フォルダー削除、パス長（OBS-11・12は対応済み 2026-09-25） |
