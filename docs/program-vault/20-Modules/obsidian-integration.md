...
- 利用者がVaultの親フォルダーを保管庫として開くと、Vaultが入れ子になる（実環境に痕跡あり、OBS-10）。親方向の `.obsidian` を検出して接続ダイアログに案内する。保存は止めず、親の設定も変更しない。
...
| OBS-10 | 中 | Vaultの入れ子の痕跡があるのに、検出していない（対応済み：検出して案内する） |
