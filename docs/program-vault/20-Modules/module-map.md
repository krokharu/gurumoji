...
| 保存・Vaultの下位関数（最下層） | `vault_files.py`: `safe_path`, `write_atomic`, `markdown`, `canonical`, `parse_frontmatter`, `research_vault_root`（ADR-121）; 書き込みの本体は `services/durable_files.write_bytes_atomically` | `tests/test_durable_files.py` |
