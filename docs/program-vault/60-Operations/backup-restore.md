...
| `<data>/media/` | 取り込んだ元メディアの管理コピー | 必須 |
| `<data>/trash/` | アプリで削除した会話（メディアと削除した行）。保持期間を過ぎると自動で完全に削除される | 必須。復元できる会話を残すなら含める |
...
- アプリで削除した会話は、保持期間（`MOJIOKOSI_TRASH_RETENTION_DAYS`、既定30日）の間は処理済みデータ一覧の「ゴミ箱」から復元できる。期間を過ぎた会話や「完全に削除」した会話は、削除前のバックアップを1組として復元する（DATA-01）。
