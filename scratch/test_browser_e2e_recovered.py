...
class BrowserJobRecoveryTests(unittest.TestCase):
    ...
    def test_real_browser_restores_the_active_job(self):
        ...
        self.assertIn('id="progress-stage-label">斁E��起こし</strong>', completed.stdout)
...
window.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    currentJob = {speaker_names: {}, speaker_profiles: {}, segments: [{
      id: 'review-test', speaker: 'A', start: 0, end: 1, text: '校正後�E本斁E,
      ai_review: {original_text: '允E�E本斁E, noise_candidate: true, fragments: [
        {reason: '候補理由 <img src=x onerror=window.reviewInjected=1>'}
      ]},
      jev_review: {original_text: '允E�E本斁E, decision: 'correction_needed',
    ...
    if (!panel || !panel.textContent.includes('ノイズ候裁E) || !panel.textContent.includes('候補理由'))
    ...
    if (!jevSummary || jevSummary.hidden || !jevSummary.textContent.includes('判定一致 1 / 1') ||
        !jevSummary.textContent.includes('両方�E�修正が忁E��E))
      throw new Error('Jev comparison summary missing');
    if (!jevPanel || !jevPanel.textContent.includes('Jev�E�修正が忁E��E) ||
        !jevPanel.textContent.includes('現行AI�E�修正あり'))
      throw new Error('Jev segment comparison missing');
    panel.querySelector('button').click();
    if (currentJob.segments[0].text !== '允E�E本斁E || segmentEditor.querySelector('textarea').value !== '允E�E本斁E)
    ...
    def test_comparison_and_unsaved_navigation_regressions(self):
        ...
        for item_id in ('compare_a', 'compare_b'):
            app.upsert_library_item(item_id=item_id, source_name=item_id,
                ...
                segments=[{'id': item_id + '_1', 'speaker': 'A', 'start': 0, 'end': 1, 'text': '?????????'}],
                speaker_names={'A': '???'}, files=[], outline=None, emotion_analysis=None,
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    expect('UX-14 hint names kept field', byId('conversation-mode-hint').textContent.includes('最多話老E��'));
